...
## The threshold was initially to low. Select another threshold more suitable
...
diplot(ardieres)
mrlplot(flows)
## A threshold around 6 should be raisonable...
...
## But, is the GP distribution suited to model our data
par(mfrow=c(1,1))
lmomplot(flows, identify = FALSE)
## Now, fit the GP distribution to exceedances above this threshold.
fitted <- fitgpd(flows, 6, 'mle',corr = TRUE)
readline("Press ENTER to continue")
## If, we want PWM estimates - unbiased one.
fitgpd(flows, 6, 'pwmu')
readline("Press ENTER to continue")
## Try also, `fitgpd(flows, 6, 'pwmb')' and `fitgpd(flows, 6, 'moments')'
## We can compute profile likelihood confidence interval...
par(mfrow=c(1,2))
gpd.pfshape(fitted, range = c(-0.1, 0.8))
gpd.pfscale(fitted, range = c(2, 7))
## Of course, classical confidence interval can be computed...
gpd.fiscale(fitted)
readline("Press ENTER to continue")
gpd.fishape(fitted)
readline("Press ENTER to continue")
par(mfrow=c(1,1),ask=TRUE)
## The same can also be obtained for return levels...
...
gpd.pfrl(fitted, 10, mu, range=c(14, 35),
         main=expression(paste("95% Profile C.I. for the 10-year return level, "
             ,mu==1.5,sep="")))
## We can produce several graphics really usefull for diagnostic of our model
